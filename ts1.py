...
max_iter = 10#số lần lặp tối đa cho thuật toán 
max_iter_LS = 5#max tối đa cho mỗi lần lặp

bounds=[b for i in range(2)]# tạo ra dải giá trị ([-10,10] ; [-10,10])
radius = 0.5 #bán kính của khu vực lân cận trong thuật toán tìm kiếm tabu.
reduce_iter = 100 #số lần lặp khi giảm bán kính của khu vực lân cận trong thuật toán.
reduce_frac = 1 #tỷ lệ giảm bán kính của khu vực lân cận trong thuật toán.
max_tabu_size = 20 # số lượng lớn nhất các vị trí trong danh sách tabu
...
# Save gif
anim.save('TS.gif', writer='imagemagick', fps=10000)